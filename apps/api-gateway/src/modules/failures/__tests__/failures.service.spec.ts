...
describe('FailuresService', () => {
  ...
  describe('findAll', () => {
    it('should return paginated failures', async () => {
      ...
      expect(mockPrisma.failures.findMany).toHaveBeenCalledWith(
  ...
  describe('findOne', () => {
    it('should return failure by id', async () => {
      ...
      expect(mockPrisma.failures.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 1, isRemoved: false } })
      );
    });
  ...
  describe('create', () => {
    it('should create a failure with creator audit fields', async () => {
      ...
      const result = await service.create({ title: 'New failure' } as any, 1);

      expect(mockPrisma.failures.create).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('remove (soft-delete)', () => {
    it('should soft-delete by setting isRemoved=true', async () => {
      ...
      expect(mockPrisma.failures.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 1 },
