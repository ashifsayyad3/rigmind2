...
export class CopilotService {
  ...
  async ask(question: string, conversationHistory: Array<{ role: 'user' | 'assistant'; content: string }>) {
    if (!this.isConfigured) throw new ServiceUnavailableException('Copilot unavailable: ANTHROPIC_API_KEY not configured');
...
9. Return raw JSON only — no markdown, no backticks, no preamble.`,
    ...
    let sqlPayload: { sql: string | null; explanation: string; intent: string };
    try {
      const raw = ((sqlResponse as any).content[0]?.text ?? '').replace(/```json|```/g, '').trim();
      sqlPayload = JSON.parse(raw);
    } catch {
      sqlPayload = { sql: null, explanation: (sqlResponse as any).content[0]?.text ?? '', intent: question };
    }
    ...
    if (!this.isSafeQuery(sqlPayload.sql)) {
      ...
      throw new BadRequestException('Query blocked: only SELECT operations are permitted');
    }
    ...
    try {
      rows = await this.prisma.$queryRawUnsafe(sqlPayload.sql) as any[];
    } catch (err: any) {
...
Highlight the most operationally significant finding first.`,
    ...
    return {
      question,
      answer: (answerResponse as any).content[0]?.text ?? '',
      sql: sqlPayload.sql,
  ...
  async generateRigReport(rigId: number) {
    if (!this.isConfigured) throw new ServiceUnavailableException('Copilot unavailable: ANTHROPIC_API_KEY not configured');
...
Open failures (${recentFailures.length}): ${JSON.stringify(recentFailures.map((f: any) => ({ title: f.title, severity: f.severity, component: f.components?.name })))}
NPT events last 90 days (${nptStats.length}): ${JSON.stringify(nptStats.map((n: any) => ({ hours: n.nptHours, category: n.delayCategory, type: n.nptType })))}
...
Structure as: Executive Summary, Key Risks (top 3), Recommended Actions (top 3), KPI Outlook.`,
    ...
    return {
      ...
      report: (report as any).content[0]?.text ?? '',
